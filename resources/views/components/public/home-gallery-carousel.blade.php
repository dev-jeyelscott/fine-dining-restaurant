...
<div data-home-gallery class="mt-14" role="region" aria-roledescription="carousel" aria-label="Restaurant gallery preview">
    @if ($galleryImages->isNotEmpty())
        ...
        <div data-home-gallery-viewport class="relative mx-auto aspect-[16/10] w-full max-w-[1080px] overflow-visible">
            <div data-home-gallery-track class="relative h-full w-full focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brand-gold" tabindex="0">
                @foreach ($galleryImages as $image)
                    <article data-home-gallery-slide data-index="{{ $loop->index }}" @class([
                        'absolute inset-y-[10%] left-1/2 h-[80%] w-[90%] -translate-x-1/2 overflow-hidden rounded-sm bg-brand-ink-soft shadow-2xl shadow-black/40 sm:w-[84%] lg:w-[78%] lg:max-w-[1080px]',
                        'z-20' => $loop->first,
                        ...
                        <div data-home-gallery-visual class="relative h-full w-full bg-brand-ink-soft">
                            @if ($image->image_url)
                                <x-public.responsive-image :image="$image" :alt="$image->alt_text ?: $image->title ?: 'Restaurant gallery image'" variant="large" sizes="(min-width: 1024px) 62vw, (min-width: 640px) 76vw, 90vw" width="1600" height="1000" img-class="h-full w-full object-cover" />
                            @endif
