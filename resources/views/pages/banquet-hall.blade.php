...
>
    ...
    <section id="banquet-details" data-gsap="section" data-banquet-details class="overflow-hidden bg-brand-ivory py-24 text-brand-ink lg:py-32">
        <div class="mx-auto grid max-w-7xl items-center gap-16 px-5 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-10">
            <div>
                ...
                <div data-gsap="panel" class="mt-9 border-l border-brand-gold bg-white/55 px-6 py-5 text-sm leading-7 text-brand-muted sm:px-7">
                    <p data-gsap-reveal class="font-semibold uppercase tracking-[0.18em] text-brand-gold-dark">Thoughtfully arranged</p>
                    <p data-gsap-reveal class="mt-3">Banquet requests are inquiries only. Our team will review your preferred date, guest count, event type, and details before confirming availability.</p>
                </div>
            </div>

            <div class="relative mx-auto w-full max-w-2xl lg:mx-0">
                <div data-gsap="frame" class="absolute -right-7 -top-7 hidden h-full w-full border border-brand-gold/70 lg:block" aria-hidden="true"></div>
                <div data-gsap="image" class="relative aspect-[4/3] overflow-hidden bg-brand-paper shadow-[0_30px_80px_rgba(23,25,22,0.18)]">
                    @if ($featuredImage?->image_url)
                        <x-public.responsive-image
                            :image="$featuredImage"
    ...
    @if ($galleryImages->isNotEmpty())
        <section data-gsap="gallery" data-banquet-gallery class="bg-brand-ink py-24 lg:py-32">
    ...
    <section data-gsap="section" data-banquet-inquiry class="relative isolate overflow-hidden bg-brand-paper py-24 text-brand-ink lg:py-32">
        @if ($featuredImage?->image_url)
            <div data-gsap="parallax" class="absolute inset-0 -z-20">
                <x-public.responsive-image
                    ...
                    width="1600"
                    height="900"
                    img-class="h-full w-full object-cover opacity-15 grayscale"
                />
            </div>
        @endif
